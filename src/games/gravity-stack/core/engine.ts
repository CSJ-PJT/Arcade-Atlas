...
export class GravityStackEngine {
  ...
  getCheckpoint(): EngineCheckpoint {
    return {
      ...this.getSnapshot(),
      ...
      pieceSequenceIndex: this.pieceSequenceIndex,
    }
  }

  restoreCheckpoint(checkpoint: EngineCheckpoint): boolean {
    if (checkpoint.snapshotVersion !== ENGINE_SNAPSHOT_VERSION || checkpoint.rulesVersion !== GRAVITY_STACK_RULES_VERSION || checkpoint.seed !== this.seed) return false
    if (!Array.isArray(checkpoint.board) || checkpoint.board.length !== BOARD_HEIGHT || checkpoint.board.some((row) => !Array.isArray(row) || row.length !== BOARD_WIDTH)) return false
    if (![checkpoint.score, checkpoint.totalClearedCells, checkpoint.maxChain, checkpoint.revision, checkpoint.rngState, checkpoint.pieceId, checkpoint.pieceSequenceIndex].every(Number.isSafeInteger) || !Number.isFinite(checkpoint.accumulatorMs)) return false
    this.board = cloneBoard(checkpoint.board)
    this.activePiece = checkpoint.activePiece ? clonePiece(checkpoint.activePiece) : null
    this.nextPiece = clonePiece(checkpoint.nextPiece)
    this.status = checkpoint.status
    this.score = checkpoint.score
    this.totalClearedCells = checkpoint.totalClearedCells
    this.maxChain = checkpoint.maxChain
    this.revision = checkpoint.revision
    this.lastWaveCount = checkpoint.lastWaveCount
    this.accumulatorMs = checkpoint.accumulatorMs
    this.pieceId = checkpoint.pieceId
    this.pieceSequenceIndex = checkpoint.pieceSequenceIndex
    this.rng.setState(checkpoint.rngState)
    return true
  }

  forceDropCells(count: number): boolean {
    if (!Number.isSafeInteger(count) || count <= 0 || this.status !== 'playing') return false
    let changed = false
