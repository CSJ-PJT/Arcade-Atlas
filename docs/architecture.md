...
`server/`의 WebSocket 서비스는 2~4명 임시 방, 참가 준비, 공통 seed와 동시 시작, 점수·레벨·생존 상태를 관리합니다. 브라우저는 순서 번호가 붙은 `left/right/rotate/down/hardDrop` 입력만 전송합니다. 서버는 브라우저와 같은 `src/games/gravity-stack/core` 엔진을 실행해 보드·점수·방전·게임오버·아이템을 판정하고 versioned checkpoint를 저장합니다. 클라이언트가 신고한 점수나 제거 셀은 받지 않습니다.
