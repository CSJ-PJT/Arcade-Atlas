...
test('direct Arcade home and catalog navigation work', async ({ page }) => {
  ...
  await page.getByRole('link', { name: /싱글 플레이/ }).click()
...
test('direct stack route supports keyboard, touch, pause, resume, and restart', async ({ page }) => {
  ...
  const startFx = page.locator('.arcade-fx-layer[data-kind="start"]')
  await expect(startFx).toBeVisible()
  await expect(startFx).toHaveCSS('pointer-events', 'none')
