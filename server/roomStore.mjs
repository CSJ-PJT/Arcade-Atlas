...
import { BOT_ENGINE_VERSION } from './gravityBot.mjs'
...
function publicPlayer(player) {
  ...
  return {
    ...
    boardPreview,
    dangerHeight,
    lastWaveCount: Number(player.engineState?.lastWaveCount) || 0,
  }
}
...
export class RoomStore {
  ...
  createRoom(name, mode = 'normal') {
    ...
    const time = this.now()
    const room = {
      code, mode: cleanMode(mode), status: 'lobby', seed: '', matchId: '', startsAt: 0,
  ...
  start(room) {
    ...
    for (const player of room.players.values()) {
      ...
      player.lastInputAt = room.startsAt
      player.lastPulseTarget = ''
    }
    this.#touch(room)
  ...
  matchState(room) {
    return { matchId: room.matchId, seed: room.seed, startsAt: room.startsAt, rulesVersion: GRAVITY_STACK_RULES_VERSION, botEngineVersion: BOT_ENGINE_VERSION }
  }

  applyEngineState(room, playerId, checkpoint, { botMoves } = {}) {
    if (room.status !== 'playing' || checkpoint?.rulesVersion !== GRAVITY_STACK_RULES_VERSION) return false
    const player = room.players.get(playerId)
    if (!player) return false
    player.score = checkpoint.score
    player.level = checkpoint.level
    player.cleared = checkpoint.totalClearedCells
    player.maxChain = checkpoint.maxChain
    player.gameStatus = checkpoint.status === 'gameOver' ? 'gameOver' : 'playing'
    player.engineState = checkpoint
    if (player.isBot && Number.isSafeInteger(botMoves) && botMoves >= player.botMoves) player.botMoves = botMoves
    if (room.mode === 'items') {
  ...
  acceptInput(room, playerId, sequence) {
    const player = room.players.get(playerId)
    if (!player || player.isBot || room.status !== 'playing' || !Number.isSafeInteger(sequence) || sequence !== player.inputSequence + 1) return false
  ...
  forfeit(room, playerId) {
    const player = room.players.get(playerId)
    if (!player || player.gameStatus !== 'playing') return false
  ...
  rematch(room, playerId) {
    ...
    for (const entry of room.players.values()) {
      ...
      entry.maxChain = 0
      entry.forfeited = false
      entry.lastInputAt = 0
      entry.lastPulseTarget = ''
      entry.botMoves = 0
    }
    this.#touch(room)
    return true
  ...
  sweep({ reconnectGraceMs = RECONNECT_GRACE_MS, roomIdleTtlMs = ROOM_IDLE_TTL_MS } = {}) {
    ...
    for (const [code, room] of this.rooms) {
      ...
      for (const player of [...room.players.values()]) {
        ...
        if (!player.isBot && player.connected && room.status === 'playing' && player.gameStatus === 'playing' && now - player.lastInputAt >= AFK_FORFEIT_MS) {
          this.forfeit(room, player.id)
          changed = true
          roomChanged = true
        }
      }
  ...
  hydrate(snapshot) {
    if (!snapshot || snapshot.version !== SNAPSHOT_VERSION || snapshot.rulesVersion !== GRAVITY_STACK_RULES_VERSION || snapshot.botEngineVersion !== BOT_ENGINE_VERSION || !Array.isArray(snapshot.rooms)) return false
    const now = this.now()
    this.rooms.clear()
    for (const source of snapshot.rooms) {
      ...
      for (const entry of source.players) {
        ...
        players.set(String(entry.id), {
          ...
          lastInputAt: Number(entry.lastInputAt) || now,
          lastPulseTarget: String(entry.lastPulseTarget ?? ''),
        })
      }
      if (players.size === 0) continue
      const room = {
        code: this.#normalizeCode(source.code), mode: cleanMode(source.mode),
