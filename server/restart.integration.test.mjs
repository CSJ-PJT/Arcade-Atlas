...
describe('Arcade server restart recovery', () => {
  it('restores a human board checkpoint, score, pieces and input revision', async () => {
    ...
    first.socket.send(JSON.stringify({ type: 'create', protocol: 2, name: 'Restart QA' }))
    const joined = await first.next((message) => message.type === 'joined')
    first.socket.send(JSON.stringify({ type: 'addBot', protocol: 2, difficulty: 'pilot' }))
    first.socket.send(JSON.stringify({ type: 'ready', protocol: 2, ready: true }))
    ...
    first.socket.send(JSON.stringify({ type: 'start', protocol: 2 }))
    ...
    first.socket.send(JSON.stringify({ type: 'input', protocol: 2, sequence: 1, command: 'hardDrop' }))
    ...
    second.socket.send(JSON.stringify({ type: 'resume', protocol: 2, code: joined.room.code, playerId: joined.playerId, reconnectToken: joined.reconnectToken }))
    ...
    second.socket.send(JSON.stringify({ type: 'leave', protocol: 2 }))
