...
describe('multiplayer room store', () => {
  ...
  it('rejects score regression and stale match progress', () => {
    ...
    expect(store.updateProgress(room, player.id, { matchId: match.matchId, score: 100, level: 99, cleared: 6, gameStatus: 'playing' })).toBe(false)
    expect(store.updateProgress(room, player.id, { matchId: match.matchId, score: 10_000, level: 1, cleared: 12, gameStatus: 'playing' })).toBe(false)
    expect(store.updateProgress(room, player.id, { matchId: match.matchId, score: 110, level: 1, cleared: 7, gameStatus: 'playing' })).toBe(false)
  ...
  it('resumes the same player within the reconnect grace without losing progress', () => {
    ...
    store.setReady(host.room, guest.player.id, true)
    const match = store.start(host.room)
    store.updateProgress(host.room, guest.player.id, { matchId: match.matchId, score: 120, level: 1, cleared: 8, gameStatus: 'playing' })
