...
describe('multiplayer room store', () => {
  ...
  it('keeps normal rooms item-free and validates item attacks on the server', () => {
    ...
    store.setReady(host.room, guest.player.id, true)
    const match = store.start(host.room)
    store.applyEngineState(host.room, host.player.id, checkpoint(match.seed, { score: 300, cleared: 36 }))
    store.applyEngineState(host.room, guest.player.id, checkpoint(match.seed, { score: 100, cleared: 12 }))
    expect(host.player.items).toEqual({ pulse: 2, shield: 1 })
    expect(guest.player.items).toEqual({ pulse: 1, shield: 0 })
    ...
    guest.player.items.shield = 1
    store.useItem(host.room, guest.player.id, 'shield')
    const blocked = store.useItem(host.room, host.player.id, 'pulse', guest.player.id)
    expect(blocked?.blocked).toBe(true)
    expect(guest.player.shielded).toBe(false)
